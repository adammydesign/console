	...
	if ($available >= $rooms) {
		...
		if ($available > $rooms) {
			...
			while ($room_rows = mysql_fetch_array($room_ids)) {
				...
				// get the current price per room
				$finalprice = $room_rows['room_price'];
				
				// loop per day per room
						...
						$linetotal = "";
							while ($days >= $increment) {
								...
								$finalprice = $room_rows['room_price'];
							//for each room per day staying check find out and echo the prices
										
										$dayprices .=  date("D", strtotime($date));
										
											// is there a room id based modifer
											...
											$modinfo = mysql_fetch_array($getroommod);
												...
												if (mysql_num_rows($getroommod) >= 1)
													{
													...
													$linetotal = $linetotal + $finalprice;
												...
												// is there a modifer for this date?
				...
				//echo table rows
				echo '<tr><td><img src="'.$image.'" /></td><td>'.$room_rows['room_name'].'</td><td>'.htmlspecialchars($room_rows['room_text']).'<br/><strong>'.$dayprices.' Total: £'.$linetotal.'</strong></td><td class="span2" style="text-align: center;"><input type="hidden" value="'.$linetotal.'" name="linetotal[]"><input type="checkbox" value="'.$room_rows['room_ID'].'" name="room[]"></td></tr>';
